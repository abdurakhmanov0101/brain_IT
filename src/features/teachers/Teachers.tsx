...
import { Users, Plus, Search, Phone, Mail, Edit2, Trash2, BookOpen, UsersRound, ChevronDown, KeyRound, Eye, EyeOff, Copy, Check } from 'lucide-react';
...
export const Teachers: React.FC = () => {
  ...
  const handleSubmit = (e: React.SyntheticEvent<HTMLFormElement>) => {
    ...
    const username = form.username || form.fullName.split(' ')[0].toLowerCase();
    const password = form.password || `${form.fullName.split(' ')[0]}@2025`;
    if (editId) {
      updateTeacher(editId, { ...form, photo, username, password });
      addToast({ type: 'success', message: `${form.fullName} ma'lumotlari yangilandi` });
    } else {
      addTeacher({ ...form, photo, username, password });
      addToast({ type: 'success', message: `${form.fullName} o'qituvchilar ro'yxatiga qo'shildi` });
    }
    setAddOpen(false);
  };

  const openCred = (t: Teacher) => {
    setCredForm({ username: t.username, password: t.password });
    setShowCredPass(false);
    setCopied(null);
    setCredId(t.id);
  };

  const handleCopy = (type: 'user' | 'pass', value: string) => {
    navigator.clipboard.writeText(value);
    setCopied(type);
    setTimeout(() => setCopied(null), 2000);
  };

  const handleSaveCred = (e: React.SyntheticEvent<HTMLFormElement>) => {
    e.preventDefault();
    if (!credForm.username || !credForm.password) return;
    updateTeacher(credId!, credForm);
    addToast({ type: 'success', message: 'Login ma\'lumotlari yangilandi' });
    setCredId(null);
  };
    ...
    <div className="space-y-6">
      <div className="flex flex-col sm:flex-row sm:items-center justify-between gap-4">
        ...
        <button onClick={openAdd}
          className="inline-flex items-center gap-2 bg-emerald-600 hover:bg-emerald-700 text-white px-4 py-2.5 rounded-xl text-sm font-semibold transition-colors shadow-lg shadow-emerald-600/20">
      ...
      <div className="grid grid-cols-2 sm:grid-cols-4 gap-4">
        ...
        <StatCard title="Jami guruhlar" value={totalGroups} icon={UsersRound} iconColor="text-emerald-600 dark:text-emerald-400" />
        <StatCard title="Jami o'quvchilar" value={totalStudents} icon={BookOpen} iconColor="text-emerald-600 dark:text-emerald-400" />
      </div>
          ...
          return (
            <div key={teacher.id}
                ...
                <div className="flex-1 min-w-0">
                  <div className="flex items-start justify-between gap-2">
                    <div className="min-w-0">
                      ...
                      <p className="text-xs text-emerald-600 dark:text-emerald-400 font-medium">{teacher.specialization}</p>
                    </div>
                    <div className="flex gap-1 shrink-0">
                      <button onClick={() => openCred(teacher)} title="Login / Parol"
              ...
              <div className="mt-4 grid grid-cols-3 gap-2 text-center">
                {[
                  { label: 'Guruh', value: groupCount, color: 'text-emerald-600 dark:text-emerald-400' },
                  { label: "O'quvchi", value: stuCount, color: 'text-emerald-600 dark:text-emerald-400' },
                  { label: 'Kurs', value: teacherCourses.length, color: 'text-emerald-600 dark:text-emerald-400' },
          ...
          <div className="grid grid-cols-1 sm:grid-cols-2 gap-4">
            ...
            <div>
              ...
              <div className="relative">
                ...
                <select value={form.specialization} onChange={(e) => setForm((f) => ({ ...f, specialization: e.target.value }))}
                  className="w-full appearance-none rounded-xl border border-slate-200 dark:border-dark-border bg-white dark:bg-dark-card py-2.5 px-3 pr-8 text-sm text-slate-900 dark:text-white focus:outline-none focus:ring-2 focus:ring-emerald-500">
            ...
            <div className="sm:col-span-2">
              <div className="flex items-center gap-2 mb-2">
                ...
                <span className="text-xs font-bold text-slate-500 dark:text-slate-400 uppercase tracking-wider">Login ma'lumotlari</span>
              </div>
              <div className="grid grid-cols-2 gap-3">
                ...
                <div>
                  ...
                  <input value={form.password} onChange={(e) => setForm((f) => ({ ...f, password: e.target.value }))}
                    placeholder="Bobur@2025"
                    className="w-full rounded-xl border border-slate-200 dark:border-dark-border bg-white dark:bg-dark-card py-2.5 px-3 text-sm text-slate-900 dark:text-white focus:outline-none focus:ring-2 focus:ring-emerald-500" />
                </div>
              </div>
              <p className="text-[10px] text-slate-400 mt-1">Bo'sh qoldirilsa avtomatik yaratiladi</p>
            </div>
          </div>
      ...
      {/* Credentials Modal */}
      <Modal open={!!credId} onClose={() => setCredId(null)} title="Login / Parol boshqaruvi" size="sm">
        {credId && (() => {
          const t = teachers.find((x) => x.id === credId);
          if (!t) return null;
          return (
            <form onSubmit={handleSaveCred} className="space-y-4">
              <div className="flex items-center gap-3 bg-slate-50 dark:bg-slate-800/40 rounded-xl p-3">
                <img src={t.photo} alt={t.fullName} className="h-10 w-10 rounded-xl object-cover" />
                <div>
                  <p className="font-bold text-sm text-slate-800 dark:text-white">{t.fullName}</p>
                  <p className="text-xs text-emerald-500">{t.specialization}</p>
                </div>
              </div>

              <div className="space-y-3">
                <div>
                  <label className="block text-xs font-semibold text-slate-500 dark:text-slate-400 mb-1.5 uppercase tracking-wider">Login</label>
                  <div className="relative">
                    <input value={credForm.username} onChange={(e) => setCredForm((f) => ({ ...f, username: e.target.value }))}
                      className="w-full rounded-xl border border-slate-200 dark:border-dark-border bg-white dark:bg-dark-card py-2.5 px-3 pr-10 text-sm text-slate-900 dark:text-white focus:outline-none focus:ring-2 focus:ring-emerald-500" />
                    <button type="button" onClick={() => handleCopy('user', credForm.username)}
                      className="absolute right-2.5 top-1/2 -translate-y-1/2 text-slate-400 hover:text-emerald-600 transition-colors">
                      {copied === 'user' ? <Check className="h-4 w-4 text-emerald-500" /> : <Copy className="h-4 w-4" />}
                    </button>
                  </div>
                </div>

                <div>
                  <label className="block text-xs font-semibold text-slate-500 dark:text-slate-400 mb-1.5 uppercase tracking-wider">Parol</label>
                  <div className="relative">
                    <input type={showCredPass ? 'text' : 'password'} value={credForm.password}
                      onChange={(e) => setCredForm((f) => ({ ...f, password: e.target.value }))}
                      className="w-full rounded-xl border border-slate-200 dark:border-dark-border bg-white dark:bg-dark-card py-2.5 px-3 pr-20 text-sm text-slate-900 dark:text-white focus:outline-none focus:ring-2 focus:ring-emerald-500" />
                    <div className="absolute right-2 top-1/2 -translate-y-1/2 flex items-center gap-1">
                      <button type="button" onClick={() => handleCopy('pass', credForm.password)}
              ...
              <div className="flex gap-3 pt-1">
                <button type="button" onClick={() => setCredId(null)}
            ...
            <div className="space-y-5">
              <div className="flex items-center gap-4">
                ...
                <div>
                  ...
                  <p className="text-sm text-emerald-600 dark:text-emerald-400 font-medium">{detailTeacher.specialization}</p>
              ...
              <div className="grid grid-cols-2 gap-3 text-sm">
                ...
                <div className="bg-slate-50 dark:bg-slate-800/50 rounded-xl p-3">
                  ...
                  <p className="font-black text-emerald-600 dark:text-emerald-400 text-xl">{tStudents.length}</p>
                </div>
              </div>
              ...
              {tCourses.length > 0 && (
                <div>
                  ...
                  <div className="flex flex-wrap gap-2">
                    {tCourses.map((c) => <Badge key={c.id} label={c.name} color="emerald" />)}
                  </div>
                </div>
              )}
              ...
              <button onClick={() => { openEdit(detailTeacher); setDetailId(null); }}
                className="w-full py-2.5 rounded-xl bg-emerald-600 hover:bg-emerald-700 text-white text-sm font-semibold">
                Tahrirlash
              </button>
            </div>
          );
