...
const COLORS = ['indigo', 'blue', 'emerald', 'violet', 'red', 'pink', 'amber', 'slate', 'cyan', 'orange'];

const emptyForm: FormState = {
  name: '', category: 'Frontend', monthlyPrice: 4000000,
  teacherPercent: 30, durationMonths: 4, lessonsPerWeek: 3, syllabus: [], color: 'indigo',
};
                  ...
                  <ol className="space-y-1.5">
                    {course.syllabus.map((item, i) => (
                      <li key={i} className="flex items-start gap-2 text-sm text-slate-600 dark:text-slate-300">
                        <span className="text-xs font-bold text-indigo-400 shrink-0 mt-0.5">{i + 1}.</span>{item}
                      </li>
                    ))}
                  </ol>
                </div>
      ...
      <Modal open={addOpen} onClose={() => setAddOpen(false)} title={editId ? 'Kursni tahrirlash' : 'Yangi kurs yaratish'} size="lg">
        <form onSubmit={handleSubmit} className="space-y-4">
          ...
          {previewLessonPrice > 0 && (
            <div className="bg-indigo-50 dark:bg-indigo-900/20 rounded-xl p-3 text-xs text-indigo-700 dark:text-indigo-300">
          ...
          <div>
            ...
            <textarea value={syllabusText} onChange={(e) => setSyllabusText(e.target.value)} rows={6} placeholder={"HTML asoslari\nCSS flexbox\nJavaScript kirish\n..."}
              className="w-full rounded-xl border border-slate-200 dark:border-dark-border bg-white dark:bg-dark-card py-2.5 px-3 text-sm text-slate-900 dark:text-white focus:outline-none focus:ring-2 focus:ring-indigo-500 resize-none font-mono" />
          ...
          <div className="flex gap-3 pt-2">
            ...
            <button type="submit" className="flex-1 py-2.5 rounded-xl bg-indigo-600 hover:bg-indigo-700 text-white text-sm font-semibold">{editId ? 'Saqlash' : 'Yaratish'}</button>
          </div>
        </form>
      </Modal>
    </div>
