  ...
  ({ studentName, courseName, issueDate, certificateNumber, verifyUrl }, ref) => {
    return (
      ...
      >
        ...
        <div className="absolute -bottom-32 -right-32 w-96 h-96 bg-yellow-500/10 rounded-full blur-[100px] pointer-events-none"></div>

        {/* PREMIUM DARK EMERALD RIBBON ON THE RIGHT */}
        <div className="absolute top-0 right-[48px] w-36 h-[72%] bg-gradient-to-b from-emerald-900 to-emerald-950 z-40 shadow-2xl flex flex-col items-center pt-20" style={{ clipPath: 'polygon(0 0, 100% 0, 100% 100%, 50% 92%, 0 100%)' }}>
            <p className="text-amber-400/90 font-bold tracking-[0.2em] text-[10px] leading-relaxed text-center mb-16">
              COURSE<br/>CERTIFICATE
            </p>
            {/* Gold Seal inside the ribbon */}
            <div className="opacity-100 drop-shadow-xl relative w-[100px] h-[100px]">
        ...
        {/* LEFT CONTENT AREA - Fixed width to strictly avoid the ribbon */}
        <div className="absolute inset-0 w-[780px] h-full flex flex-col justify-between pt-16 pb-16 px-20 z-20">
          ...
          <div className="flex justify-between items-start mb-12">
            ...
            <div className="text-right flex flex-col justify-center h-16 pt-2">
              <p className="text-[9px] text-emerald-600/80 uppercase tracking-widest font-black mb-1">Sertifikat Raqami</p>
          ...
          <div className="flex-1 flex flex-col justify-center">
            <p className="text-emerald-600 font-bold tracking-[0.3em] uppercase text-xs mb-6 flex items-center gap-4">
              Muvaffaqiyatli Bitiruv Sertifikati
              <span className="flex-1 h-[1px] bg-emerald-200"></span>
            </p>
            
            <p className="text-zinc-500 text-[13px] font-medium mb-3 uppercase tracking-widest">Ushbu sertifikat rasman taqdim etiladi:</p>

            <h1 className="text-[52px] font-black mb-6 text-emerald-950 capitalize drop-shadow-sm leading-tight" style={{ fontFamily: "'Playfair Display', serif" }}>
              {studentName}
            </h1>
            
            <p className="text-zinc-700 text-base max-w-xl leading-relaxed">
              Brain IT Academy tomonidan tashkil etilgan <span className="font-bold text-emerald-800 text-lg mx-1 border-b-[1.5px] border-emerald-300">{courseName}</span> 
          ...
          {/* Footer - Date, Signature, QR Code exactly within the left area */}
          <div className="flex justify-between items-end w-full pt-8">
            
            {/* Date */}
            <div className="flex flex-col items-center w-32">
              <div className="w-full border-b-[1.5px] border-emerald-800/20 mb-2"></div>
              <p className="text-[9px] font-black text-emerald-600/80 uppercase tracking-widest">Berilgan Sana</p>
              <p className="text-sm font-bold mt-1 text-emerald-950">{new Date(issueDate).toLocaleDateString('uz-UZ')}</p>
            </div>
            
            {/* Signature */}
            <div className="flex flex-col items-center w-48 relative">
              <div className="w-full border-b-[1.5px] border-emerald-800/20 mb-2 flex justify-center items-end h-16 relative z-10">
                {/* Temp Signature */}
                <span style={{ fontFamily: "'Brush Script MT', 'Lucida Handwriting', cursive", fontSize: '2.2rem', color: '#064e3b', opacity: 0.85, transform: 'rotate(-5deg) translateY(6px)', display: 'inline-block' }}>
                  J. Omonov
                </span>
              </div>
              <p className="text-[9px] font-black text-emerald-600/80 uppercase tracking-widest relative z-10">Akademiya Direktori</p>
              <p className="text-sm font-bold mt-1 text-emerald-950 relative z-10">Jahongir Omonov</p>
            </div>

            {/* QR Code */}
            <div className="flex flex-col items-center p-2 bg-white rounded-xl shadow-lg shadow-emerald-900/10 border border-emerald-100 shrink-0 relative z-10">
              <QRCode value={verifyUrl} size={64} level="M" fgColor="#064e3b" />
              <p className="text-[8px] text-emerald-700/70 mt-1.5 font-bold tracking-widest uppercase">Tasdiqlash</p>
            </div>
