    ...
    <Modal open={open} onClose={onClose} title="Yangi guruh yaratish" size="lg">
      <form onSubmit={handleSubmit} className="space-y-4">
          ...
          <div className="flex flex-wrap gap-2">
            {DAYS.map((day) => (
              <button key={day} type="button" onClick={() => toggleDay(day)}
                className={`px-3 py-1.5 rounded-lg text-xs font-medium transition-colors ${form.days.includes(day) ? 'bg-emerald-600 text-white' : 'bg-slate-100 dark:bg-slate-800 text-slate-600 dark:text-slate-300 hover:bg-slate-200 dark:hover:bg-slate-700'}`}>
                {day.slice(0, 3)}
              </button>
            ))}
          </div>
        ...
        <div className="flex gap-3 pt-2">
          ...
          <button type="submit" className="flex-1 py-2.5 rounded-xl bg-emerald-600 hover:bg-emerald-700 text-white text-sm font-semibold transition-colors">Guruh yaratish</button>
        </div>
      </form>
    </Modal>
  );
