  ...
  return (
    <div className="space-y-6">
      <div className="flex flex-col sm:flex-row sm:items-center justify-between gap-4">
        <div>
          <h1 className="font-heading font-black text-2xl text-slate-900 dark:text-white">Shartnomalar</h1>
          <p className="text-sm text-slate-500 dark:text-slate-400">O'quvchilar bilan shartnomalar</p>
        </div>
              ...
              {contracts.map((ct) => {
                ...
                return (
                  <tr key={ct.id} className="hover:bg-slate-50 dark:hover:bg-slate-800/30 transition-colors">
                    ...
                    <td className="px-5 py-4 font-bold text-indigo-600 dark:text-indigo-400">{ct.totalPrice.toLocaleString()} so'm</td>
      ...
      <Modal open={addOpen} onClose={() => setAddOpen(false)} title="Yangi shartnoma" size="md">
        <form onSubmit={handleCreate} className="space-y-4">
          ...
          {form.courseId && form.startDate && form.endDate && (() => {
            ...
            return <div className="bg-indigo-50 dark:bg-indigo-900/20 rounded-xl p-3 text-xs text-indigo-700 dark:text-indigo-300">{months} oy × {getCourse(form.courseId)?.monthlyPrice.toLocaleString()} = <strong>{total.toLocaleString()} so'm</strong></div>;
          })()}
          <div className="flex gap-3">
            ...
            <button type="submit" className="flex-1 py-2.5 rounded-xl bg-indigo-600 hover:bg-indigo-700 text-white text-sm font-semibold">Yaratish</button>
          </div>
        </form>
      </Modal>
          ...
          <div className="space-y-5">
            ...
            <div className="space-y-3 text-sm">
              ...
              <div className="flex justify-between border-t border-slate-200 dark:border-dark-border pt-3">
                ...
                <span className="font-black text-indigo-600 dark:text-indigo-400 text-lg">{viewContract.totalPrice.toLocaleString()} so'm</span>
              </div>
            </div>
