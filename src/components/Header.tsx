...
export const Header: React.FC<HeaderProps> = ({
  ...
  const title = getTranslation(language, activeTab as any) || PAGE_TITLES[activeTab] || 'Boshqaruv Paneli';
    ...
    <header className="h-[72px] px-4 lg:px-6 flex items-center justify-between gap-4 relative z-30 bg-light-surface dark:bg-dark-surface border-b border-light-border dark:border-dark-border shadow-sm">
      ...
      <div className="flex items-center gap-3 min-w-0">
        ...
        <div className="hidden sm:block">
          ...
          <p className="text-xs font-semibold text-zinc-500 dark:text-zinc-400 truncate mt-0.5">
            <span className="text-emerald-600 dark:text-emerald-400">{currentUser.name}</span>
      ...
      <div className="flex items-center gap-2.5 shrink-0">
        ...
        <select
          ...
          className="hidden lg:block text-xs font-bold text-zinc-600 dark:text-zinc-300 bg-transparent border-0 focus:ring-0 cursor-pointer py-1 pr-1 rounded-lg hover:text-emerald-600 dark:hover:text-emerald-400 transition-colors"
        >
        ...
        <button
          ...
          className="p-2 rounded-xl text-zinc-500 dark:text-zinc-400 hover:bg-zinc-100 dark:hover:bg-zinc-800 hover:text-emerald-600 transition-all"
        >
          {darkMode
        ...
        <div className="relative">
          <button
            ...
            className="relative p-2 rounded-xl text-zinc-500 dark:text-zinc-400 hover:bg-zinc-100 dark:hover:bg-zinc-800 hover:text-emerald-600 transition-all"
          >
            <Bell className={`h-5 w-5 ${unread > 0 ? 'text-emerald-500' : ''}`} />
            {unread > 0 && (
          ...
          {showNotif && (
            <div className="absolute right-0 mt-3 w-80 lg:w-96 card p-0 z-50 overflow-hidden shadow-2xl">
              <div className="flex items-center justify-between px-5 py-4 border-b border-light-border dark:border-dark-border">
                ...
                {unread > 0 && (
                  <button onClick={() => setNotifs(n => n.map(x => ({ ...x, read: true })))} className="text-[11px] font-bold text-emerald-600 dark:text-emerald-400 hover:underline">
                    O'qildi
                  </button>
                )}
              </div>
